...
export default function IntakePage() {
  ...
  return (
    <div
      className={`fixed inset-0 flex min-h-screen w-screen ${variant === 'client' ? 'client-auth-gradient' : 'auth-gradient'}`}
    >
      ...
      <div
        className={`flex h-screen w-full flex-col lg:w-1/2 ${variant === 'client' ? 'client-auth-gradient' : 'auth-gradient'}`}
      >
        ...
        <div className='flex-1 overflow-y-auto px-3 py-8 sm:px-4 sm:py-16'>
          ...
          <div className='w-full max-w-md mx-auto flex flex-col items-center justify-start rounded-xl py-4 px-4 sm:px-8 sm:mt-0 mt-2 relative z-10'>
            {/* Top bar for desktop */}
            <div className='hidden sm:flex w-full mb-4'>
              <AuthHeader />
            </div>

            {/* Intake Survey heading */}
            <h2
